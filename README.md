...
Предусловия | Запись о соответствующем человеке должна существовать в таблице `human`
Основной поток | <ol><li>Актор вносит информацию - имя человека</li><li>Система отображает список людей с указанным именем</li></ol>
...
Главные акторы | Администратор базы данных, Пользователь
Предусловия | Запись о соответствующем турнире должна существовать в таблице `tournament`
Основной поток | <ol><li>Актор вносит информацию - место проведения турнира</li><li>Система отображает список турниров, проведенных в указанном месте</li></ol>

### Функции:
* `modify_character_stat` - функция обновления характеристик персонажа;
* `humans_by_name` - функция получения списка людей с указанным `human.name`;
* `tournament_result_by_tournament` - функция получения таблицы результатов для турнира с указанным `tournament.id`;
* `tournament_result_by_human` - функция получения таблицы результатов для человека с указанным `human.id`;
* `tournaments_by_place` - функция получения списка турниров по месту их проведения (`tournament.place`);

### Триггеры:
* `trigger_character_stats` - триггер, срабатывающий при добавлении персонажа в `character` и устанавливающий его характеристики по умолчанию;
* `trigger_new_stat` - триггер, срабатывающий при добавлении характеристики в `character_stat_type` и устанавливающий ей значение по умолчанию у всех персонажей из того же набора правил;
* `trigger_game_event` - триггер, срабатывающий при добавлении события в `game_event` и обновляющий характеристику персонажа в соответствии с произошедшим событием;

### Индексы:
* `human(name)` - b-tree индекс на атрибут `name` в таблице `human`, позволяющий ускорить выполнение запросов, получающих `id` человека по его `name`;
